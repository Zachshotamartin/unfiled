...
**Implementation status (2026-09-02):** [ADR-0013](./decisions/ADR-0013-user-hybrid-search-trust-domain.md) is accepted and implemented on the current branch, the credential-free local aggregate is green, and the independent final audit is clear. No F PR has been opened. None of the five Vercel projects is provisioned or deployed, no account-bound provider/database/OIDC/KMS/CloudTrail proof exists, and Production remains lexical-only with BYOK disabled. Milestone G has not started.

...
The first C.5c implementation slice now includes the encrypted index lease/commit/read projections, canonical generation attestation and activation gate, strict float32 payload codec, bounded exact-scan retriever, deterministic hybrid ranking, stale-coverage repair contract, cache zeroization, and fail-closed process-wide concurrency/byte/page limits. It does not yet connect the production index worker or organizer. Credential-free evidence is 29/29 search tests, 103/103 encrypted-aggregate tests, a clean 21-file / 1,123-assertion database rebuild, zero database-lint findings, and a green repository-wide format/lint/type/coverage/boundary/OpenAPI/build/audit/routing gate.

...
**Status boundary:** C.5a–d implement the encrypted note/search overlay and the explicit plaintext-storage contract; Milestone D adds the checked-in lease-bound organizer retrieval path; and Milestone E0/E1 add the shared interaction foundation plus encrypted correction, Review resolution, and batch Undo. E2–E4 and Milestones F–G remain pending. Migration 27 is expand-compatible when installed, so a hosted database retains its pre-contract rollback columns until the separately approved operator call commits. Sections 2.1–2.3 describe the contracted target and checked-in implementation; they are local code evidence, not proof that Production has applied the contract, exercised live provider/KMS/database identities, or aged out exposed backups.
...
### 2.2 Milestone D organizer retrieval boundary

The current `unfiled_organizer_worker` database identity has exactly ten public RPCs:
...
[ADR-0011](./decisions/ADR-0011-encrypted-owner-interactions-and-personal-rules.md) and [ADR-0012](./decisions/ADR-0012-vault-only-lease-bound-byok-credentials.md) freeze migration ownership and public function names. E0 is the shared structural foundation and E1 is implemented; E2–E4 remain assigned implementation slices. This table prevents parallel lanes from choosing overlapping timestamps or generic capabilities.

| Lane | Status      | Assigned migration                                                        | Exact public functions                                                                                                                                                                                                        |
| ---- | ----------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| E2   | Pending     | `20260901000003_encrypted_routing_rules_and_personalization.sql`          | `prepare_encrypted_routing_rule_write`, `commit_encrypted_routing_rule_write`, `delete_encrypted_routing_rule`                                                                                                                |
| E3   | Pending     | `20260901000004_encrypted_generated_blocks_and_duplicate_suggestions.sql` | extend `prepare_encrypted_organizer_create`, `prepare_encrypted_organizer_append`, and `commit_encrypted_organizer_job`; add `resolve_encrypted_generated_block`                                                              |
| E4   | Pending     | `20260901000005_vault_byok_and_ai_settings.sql`                           | `get_owner_ai_settings`, `update_owner_ai_settings`, `get_user_provider_key_status`, `put_user_provider_key`, `delete_user_provider_key`, `get_lease_bound_organizer_provider_credential`                                     |

Timestamps `20260901000001` and `20260901000002` are implemented by E0 and E1. None of the assigned timestamps or names may be reused or replaced with a generic table gateway, arbitrary Vault getter, or caller-owner function.
...
E1 interactive correction, Review-resolution, and mutation-batch Undo functions are implemented as listed in §2.3. Provider-key functions and the E2–E4 rule, generated-block/duplicate, and settings/BYOK surfaces remain later-milestone work; their reserved names are not implementation evidence. Encrypted organizer create/append/Review publication and owner export/account deletion use reviewed service/worker paths; their tables remain directly non-writable to clients.
...
- Milestone E0 and E1 own implemented timestamps `20260901000001` and `00002`; pending E2, E3, and E4 retain `00003`, `00004`, and `00005`. A parallel lane may not take another lane's timestamp or rename the public RPCs frozen in §2.3.

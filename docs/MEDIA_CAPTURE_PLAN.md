...
3. The model never sees or emits attachment ids. After the plan validates, the drain appends a deterministic `append_attachments` operation with the capture's own ids. Materialization writes `![Photo](unfiled-attachment:att_…)` or `[Recording](unfiled-attachment:att_…)` into the note body and the `note_attachments` rows in the same RPC. Undo, move, and delete replay operations as they do today.

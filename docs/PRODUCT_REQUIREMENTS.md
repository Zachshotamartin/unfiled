...
- Provider, model, and effort controls expose only the `organization-model-registry-v2` allowlist (ADR-0015): OpenAI and Claude are both implemented and selectable; a model outside the selected provider is rejected, switching provider resets an incompatible model to Automatic without deleting either key, and each job snapshots the resolved provider/model/effort immutably.
- The free private beta offers no app-funded provider mode; with no usable key a capture fails closed to Inbox and the UI asks the user to add a key.

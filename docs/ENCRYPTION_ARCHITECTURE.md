...
Status: the cryptographic foundation, Milestone C capture path, and C.5a custody/expand-only boundary are implemented in code. Production account evidence and C.5b–d aggregate migration, retrieval integration, and plaintext contraction remain release blockers. This document is a security boundary, not a claim that the current note library is fully encrypted or end-to-end encrypted.
...
- Postgres `notes.title`, `notes.body_markdown`, `notes.structured_data`, revision snapshots, generated blocks, review choices, and mutation snapshots are still plaintext. C.5a adds validated envelope columns without changing these legacy writes or reads. Existing SQL full-text/literal search, Realtime mappers, and organization functions still consume plaintext. Production must not claim that notes are fully encrypted until C.5b–d migrate and contract these paths and their old backups have aged out or been cryptographically erased.
...
- C.5a also creates encrypted-RAG generation/index/job tables and race-safe service functions, but no production worker repository currently consumes them. `apps/worker` deliberately returns a content-free `503` from its drain route until the C.5c adapter is connected. No encrypted index population or retrieval claim follows from the schema alone.
...
The worker's database authority is `unfiled_index_worker`, an initially `NOLOGIN`, `NOINHERIT`, `NOBYPASSRLS` PostgreSQL role with no table, sequence, private-schema, inherited-role, or public-create privilege. After C.5c lands, a human provisions a rotated TLS-only login for that exact role. Its complete runtime surface is six `SECURITY DEFINER` RAG functions; each checks the original `session_user`, so `SET ROLE` is insufficient. A global Supabase service/secret key is forbidden in the worker deployment. [ADR-0007](./decisions/ADR-0007-dedicated-worker-database-capability-and-root-rewrap.md) records both decisions.
...
2. **Schema expansion — implemented, deliberately not cut over:** retain the already-encrypted capture path and add envelope columns for notes, revisions, generated blocks, mutation snapshots, review copies, and every other content-bearing aggregate, plus content-free RAG/job tables. Keep routing/status metadata outside the envelope only when it is truly non-sensitive. C.5b performs the resumable owner-scoped backfill; old note columns must not be dropped until verification and C.5d cutover complete.

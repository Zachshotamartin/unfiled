...
2. **Uploads are raw binary** to `POST /api/v1/captures/attachments` with a client-generated id as the idempotency key; the capture then references the ids. Reads go through `GET /api/v1/notes/{noteId}/attachments/{attachmentId}` with opaque ids only.

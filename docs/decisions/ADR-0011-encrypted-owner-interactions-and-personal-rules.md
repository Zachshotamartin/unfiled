...
Implementation status: E0–E2 are implemented locally, including private explicit/learned rules and personalization. E2's final aggregate/HTTP/PR-CI evidence remains pending. E3 generated blocks/duplicate suggestions, E4 settings/Vault-only BYOK, and Milestones F–G remain pending. The four Vercel projects are not provisioned or deployed, and this ADR does not claim account, KMS/provider, Apple-signing, archive, or physical-device evidence.
...
`20260901000001_milestone_e0_interaction_contracts.sql` implements the shared Milestone E foundation, `20260901000002_encrypted_decision_corrections.sql` implements E1, and `20260901000003_encrypted_routing_rules_and_personalization.sql` implements E2. E1 includes one migration-owned, runtime-inaccessible compatibility repair for legacy organizer receipt timestamps: it requires exact job/capture/preparation/reservation/envelope/verification attestation, changes only the relational timestamp to authoritative capture occurrence time, preserves ciphertext/revision/verification, restores the ordinary write guard, and fails the upgrade if any candidate is unattested. E3 remains unavailable to other lanes, and E4's separately governed Vault/BYOK slot and RPCs are frozen in ADR-0012.
...
E1 tests cover cross-owner denial, replay, stale revisions, canonical-anchor and membership enforcement, sorted-lock concurrency, reservation/MAC substitution, partial-failure rollback, private-key denial, plaintext-canary absence, terminal Undo, action-limited conflict Review, and the attested timestamp projection repair. E2 tests extend those properties to private rules, exact five-RPC denial, lifecycle/confirmation, observation races, canonicalization, capacity, immutable snapshots, destination hard-policy, and client replay/stale reconciliation; its final aggregate evidence is recorded only after the complete gate. E3 must still extend them to generated blocks and duplicate suggestions. No E1/E2 result establishes expansion preservation or duplicate behavior.

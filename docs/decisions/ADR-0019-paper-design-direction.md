...
7. **Spacing.** Named tokens in `UnfiledTheme` (screen padding, header-to-section distance, row padding) instead of per-screen numbers, so every tab opens the same way and pushed screens share one header distance.

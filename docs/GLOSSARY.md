...
| Quick Capture       | The WidgetKit Lock Screen extension that uses an App Intent to open a blank capture in the native iOS app. It never renders note or capture text on the Lock Screen.               |
| App Group           | The Apple entitlement shared by the native app and widget. Unfiled restricts it to content-free state: schema version, pending-capture count, and a transient random intent nonce. |

...
This directory is the planning and implementation-reference set for **Unfiled**. Milestones A, B, and the credential-free portion of Milestone C are complete. C.5a supplies encrypted-schema expansion and managed-key custody; C.5b adds typed encrypted aggregates and managed adapters; and C.5c supplies separate organizer, index-worker, and strict-decrypt-verifier runtimes around the web lifecycle controller. The organizer has a lease-linearized atomic create-or-append port, but its production planner and content-mutation cipher remain deliberately disabled until Milestone D. The production repository factory is not yet wired to the encrypted adapters, C.5d must still remove the plaintext contract, and account-bound cloud and physical-device evidence remains pending in `HUMAN_SETUP.md`. The note library is therefore not yet eligible for a complete encrypted-at-rest claim.
...
| ENCRYPTION_ARCHITECTURE.md | Capture through C.5c organizer/index/verifier audited; C.5d remains          | Milestone C.5                       |
| OPERATIONS_TEST_PLAN.md    | Current; B/C and C.5a–c boundaries recorded; human gates remain              | every milestone                     |
...
| C.5c private RAG/organizer and C.5d plaintext contraction                                                              | organizer/index/verifier implemented; D planner/C.5d pending   |
...
`apps/organizer` is a fourth Vercel trust domain, distinct from web, index worker, and verifier. Production web sends only `{"trigger":"schedule|recovery|manual"}` to its exact proved `*.vercel.app` origin with a short-lived Vercel Trusted Sources token. The organizer rejects browser sessions, cookies, `Authorization`, protection-bypass credentials, request-supplied owner IDs, broad Supabase credentials, static AWS credentials, private-manual keys, and provider keys at this checkpoint. Its own deadline is at most 49 seconds; an organization commit atomically enqueues content-free index work, and Production deliberately leaves that queue to the independent index-recovery path rather than chaining a second long-running worker request.

The exact PostgreSQL login is `unfiled_organizer_worker`: initially `NOLOGIN`, `NOINHERIT`, and `NOBYPASSRLS`, with no relation, sequence, private-schema, or public-create privilege. It can execute exactly eight job/lease-scoped functions: claim, heartbeat/revalidate, candidate projection, prepare create, prepare append, atomic commit, fail, and stale-job recovery. The database derives ownership from the live lease, issues stable IDs and key reservations, and allows one replan before forcing Review. The matching AWS workload is limited to AI-assisted object-wrap and content-MAC roots and cannot use private-manual roots, rewrap a root, or create a grant.

Local and Preview retain the deterministic in-process organizer only for synthetic development data. Production cannot fall back to that implementation. The C.5c runtime ships with an unavailable production planner and cipher by design; Milestone D must connect an evaluated provider/scoring/materialization boundary before any real create-or-append success is claimed. C.5d must then switch production reads/writes/search/export/retention to encrypted adapters, remove plaintext columns and indexes, pass canary/restore gates, and account for exposed backups. All Vercel, OIDC, AWS KMS, CloudTrail, database-login, rotation, and canary evidence remains pending in `HUMAN_SETUP.md`.

The final C.5c-3 local aggregate gate passes 449 web, 159 worker, 168 verifier, and 132 organizer tests, with organizer coverage at 87.01% statements / 83.29% branches / 93.29% functions / 90.17% lines. A clean database rebuild passes 24 files / 1,227 assertions with zero lint findings; the focused organizer suite is 42/42. The real-PostgreSQL lock-order gate passes all six cases and leaves zero fixture residue, the verifier's 1,000-document capacity gate completes in 8.18 seconds, the production dependency audit reports zero known vulnerabilities, and the unsigned native gate passes 81 Swift tests. These results do not replace the pending cloud-account, production planner/cipher, C.5d, Apple-signing, or physical-device evidence.

Local C.5b evidence established in this change set includes 93/93 encrypted-aggregate package tests, 38/38 note read/write/coordinator security tests, 14/14 adversarial note-repository tests, and 20 database test files / 1,091 assertions, including 101/101 assertions in `073_encrypted_aggregate_dual_write.test.sql`. C.5d must connect the production encrypted repository/search surface, stop plaintext writes, remove plaintext reads/search/indexes/columns, pass canary and restore gates, and age out or destroy exposed backups before any complete-library encryption-at-rest claim.

...
| ------------ | ------------------------------------------------------------ | -------------------------------------------------- | -------------------------------- | -------------------------------------------- | --------------------------------------- | ------------------------------------ |
| `local`      | Next.js + separately run isolated organizer                  | generated Xcode project + iOS 17+ Simulator        | local Supabase (CLI)             | deterministic planner only by test injection | `.env.local`, never committed           | seed fixtures only                   |
| `preview`    | Vercel web preview + isolated organizer project              | Preview configuration; signed device build manual  | isolated Supabase branch/project | organizer disabled until evaluated D adapter | Vercel preview scope + Apple Keychain   | synthetic only—never production data |
| `production` | four protected Vercel projects: web/organizer/index/verifier | Release configuration; archive and TestFlight gate | protected Supabase project       | evaluated config; organizer disabled until D | KMS/HSM + short-lived workload identity | real user data                       |
...
2. `pnpm dev` runs the web and server workspaces. Local web capture targets the separate `apps/organizer` service exactly as Preview and Production do; its normal composition has no planner/cipher until Milestone D and therefore fails closed. Deterministic organization is available only through explicit test dependency injection. `pnpm ios:generate` regenerates the Xcode project from `apps/ios/project.yml`; `pnpm ios:build` performs a code-signing-disabled Simulator build; `pnpm ios:test` runs the native test bundle on the selected iOS 17+ Simulator.
...
`organizer`: exact Production web Trusted Source plus organizer workload OIDC; no cookie, user authorization, bypass, request owner, provider key, global Supabase credential, static AWS credential, or private-manual root; exact eight-RPC TLS PostgreSQL identity; bounded claim/candidate ciphertext projections; disclosure/publication lease revalidation; strict unknown-plan authorization before database-issued ID materialization; atomic create/append/Review publication; one replan maximum; 49-second abort propagation; AI object-wrap/content-MAC custody only; and content absent from responses, errors, and logs. Production planner/cipher tests assert fail-closed unavailability until Milestone D, while deterministic implementations remain test-only.
...
The historical built-app HTTP lane's direct deterministic workflow drain is no longer a deployed composition. Current workflow integration tests inject deterministic dependencies explicitly; Local, Preview, and Production web target the isolated organizer. Until Milestone D wires the evaluated planner/cipher, the cross-process HTTP expectation is an empty-queue readiness success or a safe queued/fail-closed result—not a real create-or-append success. A Production account canary must prove a content-free web → organizer Trusted Sources call to the exact recorded organizer alias, the exact `unfiled_organizer_worker` TLS session and eight-RPC ACL, and the organizer's AI-only KMS custody. After D, the full canary must also prove that the atomic commit creates index work and that a later independent index-recovery invocation completes it; the organizer request must not chain the index worker.
...
**Milestone C.5c-3 organizer implementation evidence:** the repository contains the fourth isolated runtime, exact Trusted Sources verifier, 49-second deadline, eight-RPC database adapter/migration, lease-linearized disclosure and publication, canonical plan authorization/materialization, database-owned IDs/reservations, one-replan-then-Review policy, AI-only object-wrap/content-MAC Terraform role, and Production web caller. Production, Preview, and development invoke only the isolated organizer; the deterministic implementation is an explicit test seam, not a deployed fallback, and the Production planner/cipher remain disabled until D. The final historical credential-free aggregate gate passed 449/449 web tests at 66.46% statements / 65.72% branches / 63.30% functions / 68.75% lines, 159/159 worker tests at 93.37% statements / 90.59% branches / 97.15% functions / 95.37% lines, 168/168 verifier tests at 94.69% statements / 92.73% branches / 96.81% functions / 95.60% lines, and 132/132 organizer tests at 87.01% statements / 83.29% branches / 93.29% functions / 90.17% lines. Built worker, verifier, and organizer smoke tests, repository boundaries, and OpenAPI checks also passed. A clean database rebuild applied every migration, schema lint returned zero findings, all 24 pgTAP files / 1,227 assertions passed, and the focused organizer runtime suite contributed 42/42 assertions. The real-PostgreSQL concurrency lane passed six lock-order cases with zero fixture residue. The separately serialized verifier capacity gate opened and parsed 1,000 maximum-valid-shape encrypted documents in 8.18 seconds, the production dependency audit found zero known vulnerabilities, and the unsigned native gate passed all 81 Swift tests. These are local code, database, and simulator results only; they do not establish Vercel, AWS, database-login, CloudTrail, Apple-signing, physical-device, or live-planner evidence.

**Milestone C.5d local implementation gate (green on 2026-08-31):** rollout-aware Production note and capture composition, complete encrypted manual operations, private lexical search via authenticated `POST`, owner export/deletion, encrypted retention, and migration 27's explicit plaintext contract are implemented. A clean reset applied migrations 00–27, database lint returned zero findings, and all 32 pgTAP files / 1,417 assertions passed. The two contract suites contributed 41 assertions across zero-owner rollback/catalog/ACL/replay/fresh-signup behavior and a real nonzero owner whose key, backfill, scrub, rollout evidence, contracted state, and post-contract encrypted capture round trip were preserved. A separate concurrency test held the exact global advisory and proved the competing apply failed closed in 2 ms with no receipt or catalog change. The repository-wide format/lint/type/coverage/built-server/boundary/OpenAPI gate passed, and the Swift/Xcode gate built successfully and passed 86/86 tests on an iPhone 17 Pro simulator. This is implementation evidence only: Production has not applied the contract.

**Complete Milestone C.5 production gate (not yet green):** deploy and witness managed-KMS workload separation, exact database identities, fail-closed canaries, rotation/rewrap, restored-backup recovery, the explicit production contraction, post-contract live-store canaries, encrypted retrieval performance/quality, and pre-contract-backup expiry. Milestone D still owns real auto-routing. Any production complete-library “encrypted notes” claim remains blocked until the human evidence in `HUMAN_SETUP.md` is recorded; any E2EE claim remains prohibited.

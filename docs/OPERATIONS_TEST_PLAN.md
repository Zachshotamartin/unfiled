...
| ------------ | ------------------------------------------------------------ | -------------------------------------------------- | -------------------------------- | -------------------------------------------- | --------------------------------------- | ------------------------------------ |
| `local`      | Next.js + deterministic in-process organizer fixtures        | generated Xcode project + iOS 17+ Simulator        | local Supabase (CLI)             | mock model adapter default; real key opt-in  | `.env.local`, never committed           | seed fixtures only                   |
| `preview`    | Vercel web preview; deterministic synthetic organizer        | Preview configuration; signed device build manual  | isolated Supabase branch/project | real adapter, non-prod budget, cheap tier    | Vercel preview scope + Apple Keychain   | synthetic only—never production data |
...
2. `pnpm dev` runs the web and server workspaces. Local web capture uses only the deterministic in-process organizer against synthetic data; `apps/organizer` is also independently buildable/testable but its production composition has no planner until Milestone D. `pnpm ios:generate` regenerates the Xcode project from `apps/ios/project.yml`; `pnpm ios:build` performs a code-signing-disabled Simulator build; `pnpm ios:test` runs the native test bundle on the selected iOS 17+ Simulator.
...
That local HTTP lane intentionally exercises the deterministic in-process organizer with synthetic fixtures. It is not evidence that Production may run organization inside `apps/web`. A Production account canary must instead prove a content-free web → organizer Trusted Sources call to the exact recorded organizer alias, the exact `unfiled_organizer_worker` TLS session and eight-RPC ACL, and the organizer's AI-only KMS custody. Until Milestone D wires the evaluated planner/cipher, an empty-queue readiness call may succeed but no real create-or-append success may be claimed. After D, the full canary must also prove that the atomic commit creates index work and that a later independent index-recovery invocation completes it; the organizer request must not chain the index worker.
...
**Milestone C.5b implementation evidence:** the recorded change set implements the typed encrypted aggregate, strict service-only reservation/prepare/write/read/backfill/rollout RPC surface, managed note/capture adapters, deterministic replay comparison, verification-aware backfill, and a content-free service-only storage attestation used by the HTTP canary. Focused checks established 93/93 aggregate-package tests, 38/38 note read/write/coordinator security tests, and 14/14 adversarial note-repository tests. The complete database run applied all migrations from zero and passed 20 files / 1,091 assertions; `073_encrypted_aggregate_dual_write.test.sql` contributed 101/101. The frozen install, production dependency audit, repository-wide format/lint/type/coverage/boundary/OpenAPI gate, deterministic routing evaluation, production build, database lint, and local HTTP acceptance suite also passed. Any former-client compatibility or widget build checks in the historical run are superseded and omitted from the current gate. Private-manual encrypted capture is executable. C.5c now supplies the organizer's atomic writer substrate, but Production still fails closed until Milestone D supplies its evaluated planner/cipher and C.5d switches the production repository to encrypted adapters. These results are not native release evidence, a production cutover, or a complete C.5 gate.
...
**Milestone C.5c-3 organizer implementation evidence:** the repository contains the fourth isolated runtime, exact Trusted Sources verifier, 49-second deadline, eight-RPC database adapter/migration, lease-linearized disclosure and publication, canonical plan authorization/materialization, database-owned IDs/reservations, one-replan-then-Review policy, AI-only object-wrap/content-MAC Terraform role, and Production web caller. Local/Preview deterministic fallback is synthetic-only; Production has no fallback and keeps its planner/cipher disabled until D. The final credential-free aggregate gate passed 449/449 web tests at 66.46% statements / 65.72% branches / 63.30% functions / 68.75% lines, 159/159 worker tests at 93.37% / 90.59% / 97.15% / 95.37%, 168/168 verifier tests at 94.69% / 92.73% / 96.81% / 95.60%, and 132/132 organizer tests at 87.01% / 83.29% / 93.29% / 90.17%. Built worker, verifier, and organizer smoke tests, repository boundaries, and OpenAPI checks also passed. A clean database rebuild applied every migration, schema lint returned zero findings, all 24 pgTAP files / 1,227 assertions passed, and the focused organizer runtime suite contributed 42/42 assertions. The real-PostgreSQL concurrency lane passed six cases: row-first note, capture, and organization-job updates plus organization-job deletion all failed fast instead of deadlocking; an owner change was rejected before waiting on the new owner's advisory lock; and the canonical organizer advisory → job → capture acquisition completed. Its fixture cleanup left zero residual rows. The separately serialized verifier capacity gate opened and parsed 1,000 maximum-valid-shape encrypted documents in 8.18 seconds, the production dependency audit found zero known vulnerabilities, and the unsigned native gate passed all 81 Swift tests. These are local code, database, and simulator results only; they do not establish Vercel, AWS, database-login, CloudTrail, Apple-signing, physical-device, or live-planner evidence.

**Complete Milestone C.5 gate (not yet green):** C.5d must make REQ-E1/E2/R8/R9 and E2E-18–20 pass; connect the production encrypted repository and private RAG search paths; complete the plaintext schema/index contract; witness managed-KMS separation, fail-closed behavior, rewrap, and restore; and pass encrypted retrieval performance/quality gates. Milestone D still owns real auto-routing, while any complete-library “encrypted notes” product claim remains blocked through C.5d and account evidence.

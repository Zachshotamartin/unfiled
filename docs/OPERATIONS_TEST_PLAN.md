...
| 11 infrastructure         | Terraform format/validate/test for four KMS root families, three exact-subject OIDC roles, context-bound policy, rotation, and workload isolation | Linux  | yes      |
...
**C.5 account evidence (pending):** deploy the exact web, worker, and verifier Vercel projects; prove each one-way Production Trusted Sources mapping and workload OIDC exchange; prove the worker can generate and decrypt against only the active AI object-wrap root while the verifier can decrypt only active/retired AI object-wrap roots; directly deny private, content-MAC, re-encryption, and data-key capabilities outside each role's allowlist; retain matching content-free CloudTrail management events; connect through hostname- and certificate-verified TLS with the exact `unfiled_index_worker` and `unfiled_rag_verifier` sessions; prove their respective six-RPC and two-RPC surfaces; and complete outage, rotation/rewrap, rollback, and restored-backup drills. `HUMAN_SETUP.md` owns the exact procedure.
...
**Milestone C.5c index/generation/verifier implementation evidence:** the current slice composes the production index worker, strict OpenAI embedding adapter, encrypted note/index custody, exact six-RPC PostgreSQL executor, content-free web lifecycle controller, and separate two-RPC strict-decrypt verifier. Maintenance can create/resume/replace, seed, drain, verify, and activate a shadow generation with replay-safe CAS behavior; deterministic invalid ciphertext fails the generation while transient verifier failures remain retryable. The verifier has a distinct Vercel/database/AWS identity, accepts only the proved Production web caller, decrypts only AI object-wrap keys, and returns no note-derived content. Its fixed 33-page, 8 MiB/page bounds provide 1,023 physical worst-row slots with admission deliberately capped at 1,000 notes; a separate four-key-record bound caps KMS work, and neither bound is environment-tunable. Focused checks pass 158/158 worker tests at 93.25% statements / 90.57% branches / 97.12% functions / 95.23% lines, 436/436 web tests, and 168/168 verifier unit tests at 94.70% statements / 92.73% branches / 96.81% functions / 95.61% lines. The separately serialized capacity gate strictly opens and parses 1,000 maximum-valid-shape encrypted documents across 30 actual pages (29 × 34 rows plus 14 rows) inside the same 18-second in-process allowance reserved by Production configuration, while separately proving the conservative 33-page database bound. A clean database rebuild applies every migration, schema lint reports zero errors, and 23 pgTAP files / 1,185 assertions pass. The 53-assertion generation-control suite covers 1,000-note admission, 1,001-note deferral, the exact largest permitted terminal ciphertext page (8,355,840 bytes), and full 1,000-row attestation/verification under the 250 ms statement budget, plus digest-tamper and stale-revision rejection. Terraform validates and passes 13 policy/rotation/guardrail tests. Production OIDC/KMS/database evidence remains human-owned; the atomic organizer and C.5d contraction remain launch-blocking.

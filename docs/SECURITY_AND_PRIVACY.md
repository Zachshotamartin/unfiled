...
| T24 | Organizer credential escapes its owner/lease scope    | B2/B3/B4 | exact `unfiled_organizer_worker` login; no relation or private-schema access; ten-RPC allowlist; owner derived from a live lease; AI-assisted projections only                          | organizer role/RPC/lease tests             |
...
- [ ] Organizer uses the exact Production web caller plus its own OIDC workload identity and dedicated OpenAI project/service-account key; no generic provider key or user BYOK is present
- [ ] Organizer database session is exactly `unfiled_organizer_worker` over verified TLS, has only the ten-RPC allowlist, and is denied tables, private schema, index-worker/verifier/admin functions, role inheritance, and RLS bypass

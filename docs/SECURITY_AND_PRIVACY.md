...
- The C.5c worker adapter must connect as the exact `unfiled_index_worker` PostgreSQL login over hostname- and certificate-verified TLS. That role has no RLS bypass, inheritance, relation access, or administrative RPC and must never receive membership in `service_role` or another parent role.

...
**Implementation status:** captures have an application-crypto foundation; C.5a implements the expand-only schema, managed-key/workload contracts, isolated worker, dedicated non-bypass worker database role, and root-rewrap CAS boundary; C.5b implements the typed encrypted aggregate, managed note/capture adapters, and verified rollout/backfill machinery through `encrypted_read`; and the C.5c index-maintenance worker plus content-free web caller are implemented. The production repository factory is not yet wired to C.5b. Fresh AI-assisted aggregate capture deliberately fails closed until the separate organizer adds its atomic encrypted writer; generation seed/strict-decrypt verification and C.5d plaintext contraction also remain open. Production account, rotation, restore, Apple-signing, and physical-device evidence remains pending. Unfiled is not E2EE because authorized application services can decrypt content.
...
The accepted RAG path pages and exact-scans only the authenticated user's encrypted active-generation documents with bounded concurrency and memory. C.5a implements ciphertext-only tables and transaction/race invariants. C.5c implements the ciphertext/key projections, generation attestation/activation gate, strict float32 payload codec, deterministic hybrid ranking, bounded exact-scan library, exact-role production index worker, and OIDC-authenticated content-free caller. The worker revalidates privacy/revision/lease immediately before provider disclosure and again before ciphertext publication; private-manual notes cannot be claimed. Persisted lexical features, snippets, and embeddings share one ciphertext envelope; non-finite values, dimension/model mismatch, malformed ciphertext, and oversized features fail closed. Existing-library generation seed, independent strict-decrypt verification, and organizer completion remain pending; incomplete coverage disables RAG-based auto-apply. Plaintext pgvector/FTS requires a future ADR and privacy review.

...
The accepted RAG path pages and exact-scans only the authenticated user's encrypted active-generation documents with bounded concurrency and memory. C.5a implements the ciphertext-only tables and transaction/race invariants. The first C.5c slice implements the ciphertext/key projections, generation attestation/activation gate, strict float32 payload codec, deterministic hybrid ranking, and bounded exact-scan retrieval library; production worker wiring, organizer completion, and index population remain pending. Persisted lexical features, snippets, and embeddings share one ciphertext envelope; bounded float32 embeddings use a strict versioned binary encoding and reject non-finite values, dimension/model mismatch, or oversized features. Private notes never receive an index row. Stale rows are ineligible, privacy/delete changes exclude immediately, and incomplete coverage disables RAG-based auto-apply. Plaintext pgvector/FTS requires a future ADR and privacy review.

...
Milestone F implements—but does not yet deploy—the fifth Vercel search service accepted by [ADR-0013](./decisions/ADR-0013-user-hybrid-search-trust-domain.md) for explicitly AI-assisted semantic requests. Its exact one-use ticket, five-RPC database login, decrypt-only AI object-wrap role, fixed provider/model, no-cache/no-content-log boundary, generation/cursor revalidation, and lexical-only fallback are present in code; final credential-free validation, required PR checks, and account-bound deployment evidence are still pending. Default, mixed, and private-manual queries remain owner-authorized lexical-only, Production remains lexical-only until every release gate passes, and Milestone D's routing-query embedding does not authorize user search traffic.

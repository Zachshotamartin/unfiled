-- Treat SQL wildcard characters as literal user input in note search.
...
begin
  ...
  with candidates as (
    select
      ...
      concat_ws(
        ' ',
        note_record.title,
        note_record.body_markdown,
        note_record.updated_at::date::text,
        parent_space.name,
        child_space.name
      ) as document_value
  ...
  select
    ...
    left(
      regexp_replace(
        case
          when position(normalized_query in lower(candidate.body_markdown)) > 0
          then substring(
            candidate.body_markdown
            from greatest(position(normalized_query in lower(candidate.body_markdown)) - 80, 1)
            for 320
          )
          else candidate.body_markdown
        end,
        '[[:space:]]+',
        ' ',
        'g'
    ...
    (
      case
        when lower(candidate.title) = normalized_query then 400.0
        when starts_with(lower(candidate.title), normalized_query) then 300.0
        when position(' ' || normalized_query in lower(candidate.title)) > 0 then 250.0
        else 100.0
      end
      + ts_rank(
        to_tsvector('simple', candidate.document_value),
        search_query
      )::double precision
    )
  from candidates as candidate
  where position(normalized_query in lower(candidate.document_value)) > 0

...
const HARD_BODY_LIMIT_BYTES = 4_096;
...
const composition = createVerifierComposition(config);
const server = createServer((incoming, outgoing) => {
  void webRequest(incoming)
    .then((request) => composition.app(request))
    ...
    .catch(() => {
      ...
      outgoing.end('{"code":"provider_unavailable","message":"The verifier is unavailable."}');
    });
});
